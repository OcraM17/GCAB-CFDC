...
class Mlp(nn.Module):
    ...
    def forward(self, x, gc1=None, gc2=None, gc3=None):
        if gc1 is not None:
            x = x * gc1.view(1, 1, -1).expand_as(x)
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
        if gc2 is not None:
            x = x * gc2.view(1, 1, -1).expand_as(x)
        x = self.fc2(x)
        if gc3 is not None:
            x = x * gc3.view(1, 1, -1).expand_as(x)
        x = self.drop(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x, s=None, gcab=None, mask_heads=None, c=1, attn_mask=None):
        if isinstance(self.attn, ClassAttention):
            cls_token = x[:, :1]
            xx = self.norm1(x)
            xx, attn, v, masks = self.attn(
                xx,
                s,
                c,
                gcab,
            ...
            cls_token = self.drop_path(xx[:, :1]) + cls_token
            c = torch.autograd.Variable(torch.LongTensor([c - 1]).cuda(), volatile=False)
            if len(gcab.nb_classes_per_task) >= 2 and c < len(gcab.nb_classes_per_task) - 1:
                gc0 = torch.where(gcab.gate(s * gcab.embs_0(c)) > 0.5, 1.0, 0.0)
                gc2 = torch.where(gcab.gate(s * gcab.embs_2(c)) > 0.5, 1.0, 0.0)
            else:
                gc0 = gcab.gate(s * gcab.embs_0(c))
                gc2 = gcab.gate(s * gcab.embs_2(c))
            cls_token = self.drop_path(self.mlp(self.norm2(cls_token), gc0, gc2, gc0)) + cls_token
            masks += [gc0, gc2]

            return cls_token, attn, v, masks
...
class ClassAttention(nn.Module):
    ...
    # with slight modifications to do Gated Class Attention
    ...
    def forward(self, x, s, c, gcab, mask_heads=None, **kwargs):
        B, N, C = x.shape
        c = torch.autograd.Variable(torch.LongTensor([c - 1]).cuda(), volatile=False)
        if len(gcab.nb_classes_per_task) >= 2 and c < len(gcab.nb_classes_per_task) - 1:
            gc0 = torch.where(gcab.gate(s * gcab.embs_0(c)) > 0.5, 1.0, 0.0)
        ...
        q = self.q(x[:, 0] * gc0.view(1, -1).expand_as(x[:, 0])).unsqueeze(1).reshape(B, 1, self.num_heads,
                                                                                      C // self.num_heads).permute(0, 2,
                                                                                                                   1, 3)
        q = q * gc0.view(1, self.num_heads, 1, -1).expand_as(q)

        x_b = x * gc0.view(1, 1, -1).expand_as(x)
        k = self.k(x_b).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = k * gc0.view(1, self.num_heads, 1, -1).expand_as(k)

        q = q * self.scale
        v = self.v(x_b).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        x_cls = x_cls * gc0.view(1, 1, -1).expand_as(x_cls)
...
class JointCA(ClassAttention):
    ...
    def forward(self, x, s, c=None):
        B, N, C = x.shape
        q = self.q(x[:, :c]).reshape(B, c, self.num_heads, C // self.num_heads).permute(0, 2,
                                                                                        1, 3)
        ...
        x_cls = (attn @ v).transpose(1, 2).reshape(B, c, C)
        x_cls = self.proj(x_cls)
        x_cls = self.proj_drop(x_cls)
